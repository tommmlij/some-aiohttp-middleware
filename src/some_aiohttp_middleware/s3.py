...
class S3Config(BaseSettings):
    ...
    def __init__(self, name: str = "default", region: str = "us-east-1", **kwargs):
        super().__init__(_env_prefix=f"S_{name.upper()}_" if name else "S_",
                         name=name, region=region, **kwargs)
...
class S3(MiddlewareBase):
    ...
    async def handle(request: Request, *args, **kwargs) -> Request:
        ...
        for s3_config in s3_configs:
            if not app.get("s3"):
                s3 = await Session().resource('s3', region_name=s3_config.region).__aenter__()
                app["s3"] = {
                    "sessions": {
                        s3_config.region: s3
                    },
                    "buckets": {
                        s3_config.name: await s3.Bucket(s3_config.bucket)
                    }
                }
            else:
